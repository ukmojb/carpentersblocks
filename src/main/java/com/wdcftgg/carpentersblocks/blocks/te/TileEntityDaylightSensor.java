...
import com.wdcftgg.carpentersblocks.blocks.BlockDaylightSensor;
...
import static com.wdcftgg.carpentersblocks.blocks.BlockDaylightSensor.POWER;

public class TileEntityDaylightSensor extends TileEntity implements ITickable,ICustomRenderBase {
    ...
    @Override
    public void update() {
        System.out.println(this.world.getBlockState(this.pos).getValue(POWER));
        ...
        if (this.world != null && !this.world.isRemote && this.world.getTotalWorldTime() % 20L == 0L)
        {
            ...
            if (this.blockType instanceof BlockDaylightSensor)
            {
                ((BlockDaylightSensor)this.blockType).updatePower(this.world, this.pos);
            }
        }
    }
