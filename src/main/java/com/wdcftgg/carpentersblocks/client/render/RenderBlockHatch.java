...
public class RenderBlockHatch extends TileEntitySpecialRenderer<TileEntityHatch> {

    private final ModelHatchBase hatchTop = new ModelHatchTop();
    private final ModelHatchBase hatchBottom = new ModelHatchBottom();
    private final ModelHatchBase hatchOpen = new ModelHatchOpen();
    private ModelHatchBase model = null;


    @Override
    public void render(TileEntityHatch te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {

        World world = te.getWorld();
        IBlockState bed = world.getBlockState(te.getPos());
        EnumFacing facing = world.getBlockState(te.getPos()).getValue(BlockHatch.FACING);
        BlockTrapDoor.DoorHalf half = world.getBlockState(te.getPos()).getValue(BlockHatch.HALF);
        boolean open = world.getBlockState(te.getPos()).getValue(BlockHatch.OPEN);
...
//        this.model = facing.equals(EnumFacing.UP) ? torchGround : torchWall;
        ...
        this.model = half.equals(BlockTrapDoor.DoorHalf.TOP) ? hatchTop : hatchBottom;
        if (open) this.model = hatchOpen;
    ...
    private void renderPiece(double x, double y, double z, int face, float alpha, TileEntityHatch te)
    {
//        this.model.preparePiece(p_193847_1_);
    ...
    private void renderTorch(ModelHatchBase modelBase, TileEntityHatch te) {
        renderOther(modelBase, te);
        renderWood(modelBase, te);
    }

    private void renderOther(ModelHatchBase modelTorchBase, TileEntityHatch te) {
        ResourceLocation res = new ResourceLocation(CarpentersBlocks.MODID + ":textures/model/hatch.png");
        this.bindTexture(res);
        modelTorchBase.renderOther(0.0625F);
    }

    private void renderWood(ModelHatchBase modelTorchBase, TileEntityHatch te) {
        ...
        if (!te.hasBlock()) {
            res = new ResourceLocation(CarpentersBlocks.MODID + ":textures/model/hatch.png");
        } else {
        ...
        modelTorchBase.renderWood(0.0625F);
