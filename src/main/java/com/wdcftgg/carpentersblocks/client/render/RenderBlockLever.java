...
public class RenderBlockLever extends TileEntitySpecialRenderer<TileEntityLever> {

    private final ModelLeverBase leverOn = new ModelLeverOn();
    private final ModelLeverBase leverOff = new ModelLeverOff();
    private ModelLeverBase model = null;


    @Override
    public void render(TileEntityLever te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {

        World world = te.getWorld();
        IBlockState lever = world.getBlockState(te.getPos());
        boolean power = lever.getValue(BlockLever.POWERED);
...
//            }
        ...
        this.model = power ? leverOn : leverOff;
    ...
    private void renderPiece(double x, double y, double z, int face, float alpha, TileEntityLever te)
    {
...
//            f2 = 0.5F;
    ...
    private void renderTorch(ModelLeverBase modelBase, TileEntityLever te) {
        renderOther(modelBase, te);
        renderWood(modelBase, te);
    }

    private void renderOther(ModelLeverBase modelTorchBase, TileEntityLever te) {
        ResourceLocation res = new ResourceLocation(CarpentersBlocks.MODID + ":textures/model/lever.png");
        this.bindTexture(res);
        modelTorchBase.renderOther(0.0625F);
    }

    private void renderWood(ModelLeverBase modelTorchBase, TileEntityLever te) {
        ...
        if (!te.hasBlock()) {
            res = new ResourceLocation(CarpentersBlocks.MODID + ":textures/model/lever.png");
        } else {
