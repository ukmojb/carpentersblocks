...
import com.wdcftgg.carpentersblocks.blocks.te.TileEntityTorch;
import com.wdcftgg.carpentersblocks.client.model.torch.ModelTorchBase;
...
public class RenderBlockTorch extends TileEntitySpecialRenderer<TileEntityTorch> {

    private final ModelTorchBase torchWall = new ModelTorchWall();
    private final ModelTorchBase torchGround = new ModelTorchGround();
    private ModelTorchBase model = null;


    @Override
    public void render(TileEntityTorch te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {

        World world = te.getWorld();
        IBlockState bed = world.getBlockState(te.getPos());
        EnumFacing facing = world.getBlockState(te.getPos()).getValue(BlockTorch.FACING);
...
//            }
    ...
    private void renderPiece(double x, double y, double z, int face, float alpha, TileEntityTorch te)
    {
...
//            f2 = 0.5F;
    ...
    private void renderTorch(ModelTorchBase modelBase, TileEntityTorch te) {
        renderOther(modelBase, te);
        renderWood(modelBase, te);
    }

    private void renderOther(ModelTorchBase modelTorchBase, TileEntityTorch te) {
        ResourceLocation res = new ResourceLocation(CarpentersBlocks.MODID + ":textures/model/torch.png");
        this.bindTexture(res);
        modelTorchBase.renderOther(0.0625F);
    }

    private void renderWood(ModelTorchBase modelTorchBase, TileEntityTorch te) {
        ...
        if (!te.hasBlock()) {
            res = new ResourceLocation(CarpentersBlocks.MODID + ":textures/model/torch.png");
        } else {
